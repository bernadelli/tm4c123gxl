...
void DMA_Init(void){
    ...
    for(i=0; i<256; i++){
        ...
        UDMA_CTLBASE_R = (int)ucControlTable;      // 9.3.1 - 3. Location of the channel control table.
        UDMA_PRIOCLR_R |= BIT30;                   // 9.3.2.1 - 1. default, not high priority
        UDMA_ALTCLR_R |= BIT30;                    // 9.3.2.1 - 2. use primary control
        UDMA_USEBURSTCLR_R |= BIT30;               // 9.3.2.1 - 3. responds to both burst and single requests
        UDMA_REQMASKCLR_R |= BIT30;                // 9.3.2.1 - 4. allow the µDMA controller to recognize requests for this channel
...
void DMA_Transfer(unsigned int *source, unsigned int *destination, unsigned int count){

    ucControlTable[CH30]   = (unsigned int)source+count*4-1;       // 9.3.2.2 - 1. Source end pointer
    ucControlTable[CH30+1] = (unsigned int)destination+count*4-1;  // 9.3.2.2 - 2. Destination end pointer
    ucControlTable[CH30+2] = 0xAA00C002+((count-1)<<4);   // 9.3.2.2 - 3. DMA Channel Control Word (DMACHCTL)
    ...
    UDMA_ENASET_R = BIT30;    // 9.3.2.3 - 1. µDMA Channel 30 is enabled.
    UDMA_SWREQ_R = BIT30;     // 9.3.2.3 - 2. software start,
